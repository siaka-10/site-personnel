...
$con= new PDO('mysql:host=localhost;dbname=bases_pscpd','root','');
 ...
 if(isset($_POST['btn_enregistrer'])){
    ...
    $sol0 =$req0->execute();
     ...
     $req = $con->prepare('INSERT INTO stagiaire VALUES(null,:nom_prenom,:nationalite,:filiere,:telephone)');

     $req->bindParam(':nom_prenom',$_POST['sai_nom_prenom']);
     $req->bindParam(':nationalite',$_POST['sai_nationalite']);
     $req->bindParam(':filiere',$_POST['sai_filiere']);
  ...
  if (isset($_POST['btn_supprimer'])) // si je clique sur le bouton supprimer
{
  ...
  $req = $con->Prepare('DELETE  FROM stagiaire WHERE code_stagiaire=:code_stagiaire');
  $req->bindParam(':code_stagiaire', $_POST['sai_code_stagiaire']);
  $sol=$req->execute();
...
if (isset($_POST['btn_modifier'])) // si je clique sur le bouton modifier
{
  ...
  $req= $con->Prepare('UPDATE stagiaire SET nom_prenom=:nom_prenom,nationalite=:nationalite,filiere=:filiere,telephone=:telephone WHERE code_stagiaire=:code_stagiaire');
...
if (isset($_POST['btn_rechercher'])) // si je clique sur le bouton rechercher
{
  ...
  $req = $con->Prepare('SELECT * FROM stagiaire WHERE code_stagiaire=:code_stagiaire');
  $req->bindParam(':code_stagiaire', $_POST['sai_rechercher']);
  $req->execute();
  $sol=$req->fetchAll();
  if(!empty($sol)){
  
      $code = $sol[0]['code_stagiaire'];
            ...
            <div class="card card-warning">
              ...
              <div class="card-body">
                <form action="" method="POST" >
                  ...
                  <div class="form-group">
                    <label class="col-form-label" for="inputError"><i class="far fa-times-circle"></i> Filière </label>
                    <input type="text" class="form-control is-valid"  name="sai_filiere">
                  </div>
